...
export default function ClayComponent() {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && containerRef.current) {
            ...
            console.log(`0 - 4`, Math.abs(0 - 4));

            const renderBuildingWithThree = (array: any[]) => {
                ...
                for (let i = 0; i < array.length; i++) {
                    ...
                    for (let j = 0; j < coordinates.length; j++) {
                        if (j === coordinates.length - 1) {
                            ...
                            var wallWidth: number = 0;
                            // var wallDepth: number = 0;

                            if (coordinates[j][0] != coordinates[j + 1][0]) {
                                wallWidth = Math.abs(coordinates[j][0] - coordinates[j + 1][0]);
                            }
                            else {
                                wallWidth = Math.abs(coordinates[j][1] - coordinates[j + 1][1]);
                            }
                            ...
                            wall.position.x = (coordinates[j][0] + (coordinates[j + 1][0] - coordinates[j][0]) / 2);
                            wall.position.y = wallHeight / 2;
                            wall.position.z = (coordinates[j][1] + (coordinates[j + 1][1] - coordinates[j][1]) / 2);
            ...
            renderBuildingWithThree(wallsArray);
